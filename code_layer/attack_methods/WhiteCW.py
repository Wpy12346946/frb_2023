...
class WhiteCW(Attack, LabelMixin):
    """
    ...
    def __init__(self, predict, num_classes, confidence=0,
                 ...
                 clip_min=0., clip_max=1., loss_fn=None,attack_type='cdr'):
    ...
    def _loss_fn(self, cls_output, rec_output, cls_y_onehot, d_output, l2distsq, const):
        ## l2
        loss = (l2distsq).sum()

        ## classifier
        if 'c' in self.attack_type:
            real = (cls_y_onehot * cls_output).sum(dim=1)
            other = ((1.0 - cls_y_onehot) * cls_output - (cls_y_onehot * TARGET_MULT)
                    ).max(1)[0]
            if self.targeted:
        ...
        ## rectifier
        if 'r' in self.attack_type:
            real = (cls_y_onehot * rec_output).sum(dim=1)
            other = ((1.0 - cls_y_onehot) * rec_output - (cls_y_onehot * TARGET_MULT)
                    ).max(1)[0]
            if self.targeted:
                loss2 = clamp(other - real + self.confidence, min=0.)
            else:
                loss2 = clamp(real - other + self.confidence, min=0.)
            loss2 = torch.sum(5 * const * loss2)
            loss+=loss2

        ## detector
        if 'd' in self.attack_type:
            loss3 = torch.sum(2 * const * (-d_output[:,0]+self.confidence))
            loss+=loss3
    ...
    #     # cls_y_onehot = torch.cat([cls_y_onehot,torch.zeros(b,1).to(device)],dim=1)
    #     cls_logit_max = cls_output.max(1)[0].unsqueeze(1)
    #     rec_logit_max = rec_output.max(1)[0].unsqueeze(1)
    #     d_prob = 1.5 - torch.softmax(d_output,1)[:,0]
    #     d_prob = d_prob.unsqueeze(1)

    #     cls_output = torch.cat([cls_output,d_prob*cls_logit_max],dim=1)
    #     rec_output = torch.cat([rec_output,d_prob*rec_logit_max],dim=1)

    #     real = (cls_y_onehot * cls_output).sum(dim=1)
    #     # TODO: make loss modular, write a loss class
    #     other = ((1.0 - cls_y_onehot) * cls_output - (cls_y_onehot * TARGET_MULT)
    #              ).max(1)[0]
    #     # - (cls_y_onehot * TARGET_MULT) is for the true label not to be selected
    ...
    #     ## rec loss
    #     rec_real = (cls_y_onehot * rec_output).sum(dim=1)
    #     # TODO: make loss modular, write a loss class
    #     rec_other = ((1.0 - cls_y_onehot) * rec_output - (cls_y_onehot * TARGET_MULT)
    #              ).max(1)[0]
    #     if self.targeted:
    #         loss3 = clamp(rec_other - rec_real + self.confidence, min=0.)
    #     else:
    #         loss3 = clamp(rec_real - rec_other + self.confidence, min=0.)
    #     loss3 = torch.sum(const * loss3)
    ...
    def _forward_and_update_delta(
        ...
        cls_output = self.predict.classifier_output
        cls_y_onehot = y_onehot
        rec_output = self.predict.rectifier_output
        d_output = self.predict.detector_output
        l2distsq = calc_l2distsq(adv, transimgs_rescale)
        loss = self._loss_fn(cls_output,rec_output, cls_y_onehot, d_output, l2distsq, loss_coeffs)
        loss.backward()
        optimizer.step()

        return loss.item(), l2distsq.data, cls_output.data, adv.data
...
def white_cw_attack(model, data, label, num_classes, targeted=False, learning_rate=0.01, max_iterations=10000, confidence=0,attack_type='cdr'):
    adversary = WhiteCW(model, num_classes, confidence=confidence, targeted=targeted,
                    learning_rate=learning_rate, binary_search_steps=4, max_iterations=max_iterations,
                    abort_early=True, initial_const=0.001, clip_min=0.0, clip_max=1.0,
                    loss_fn=None,attack_type=attack_type)
    adv_untargeted = adversary.perturb(data, label)
