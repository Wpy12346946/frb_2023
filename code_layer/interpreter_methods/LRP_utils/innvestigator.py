...
class InnvestigateModel(torch.nn.Module):
    ...
    def __init__(self, the_model,device, lrp_exponent=1, beta=.5, epsilon=1e-6,
        ...
        self.handles=self.register_hooks(self.model)
    ...
    def register_hooks(self, parent_module):
        ...
        for mod in parent_module.children():
            if list(mod.children()):
                handles=handles+self.register_hooks(mod)
                continue
            handles.append(mod.register_forward_hook(
                self.inverter.get_layer_fwd_hook(mod)))
            if isinstance(mod, torch.nn.ReLU):
                handles.append(mod.register_backward_hook(
    ...
    #             self.inverter.get_layer_fwd_hook(module))
    ...
    def evaluate(self, in_tensor):
        ...
        self.probout = torch.softmax(self.prediction,dim=1)
    ...
    def innvestigate(self, in_tensor=None, rel_for_class=None):
        ...
        with torch.no_grad():
            ...
            if rel_for_class is None:
                ...
                org_shape = self.probout.size()
                # Make sure shape is just a 1D vector per batch example.
                self.probout = self.probout.view(org_shape[0], -1)
                max_v, _ = torch.max(self.probout, dim=1, keepdim=True)
                only_max_score = torch.zeros_like(self.probout).to(self.device)
                only_max_score[max_v == self.probout] = self.probout[max_v == self.probout]
                relevance_tensor = only_max_score.view(org_shape)
                self.probout = self.probout.view(org_shape)

            else:
                org_shape = self.probout.size()
                self.probout = self.probout.view(org_shape[0], -1)
                only_max_score = torch.zeros_like(self.probout).to(self.device)
                only_max_score[:, rel_for_class] += self.probout[:, rel_for_class]
                relevance_tensor = only_max_score.view(org_shape)
                self.probout = self.probout.view(org_shape)
