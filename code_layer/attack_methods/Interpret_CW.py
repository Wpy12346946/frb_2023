...
class I_CW(Attack, LabelMixin):
    ...
    def _loss_fn(self, output, y_onehot, d_output, l2distsq, const):
    ...
    def _forward_and_update_delta(
        ...
        d_output = self.predict.detector_output.clone()
        l2distsq = calc_l2distsq(adv, transimgs_rescale)
        loss = self._loss_fn(output, y_onehot, d_output, l2distsq, loss_coeffs)
        loss.backward()
        optimizer.step()
...
def interpret_cw_attack(model, data, label, num_classes, targeted=False, learning_rate=0.01, max_iterations=10000, confidence=0):
    adversary = I_CW(model, num_classes, confidence=confidence, targeted=targeted,
