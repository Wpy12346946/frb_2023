...
from detectors.advJudge import *
...
import sklearn
def detect(opt,interpret_methods=['DL','LRP','org','VG','GBP','IG'],reverse_list=[],train = True):
...
def generate_interpret_only_rec(lb):
    ...
    dataset = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{lb}.npy')
    interpret = []
    ...
    torch.save(n,f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-{lb}.npy')
    return
...
def get_Z(ensembled_clf,reducers,interpret_methods=['DL','LRP','VG','GBP','IG','org']):
    for lb in range(opt.classifier_classes):
        for opt.interpret_method in interpret_methods:
            if opt.interpret_method == 'org':
                interpret = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{lb}.npy')
            else:
                interpret = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-{lb}.npy')
            reducer = reducers[opt.interpret_method]
            ...
            make_dir(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}')
            np.save(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-Z-{lb}.npy',Z)

    for func in opt.rec_methods:
        for opt.interpret_method in interpret_methods:
            if opt.interpret_method == 'org':
                interpret = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{func}.npy')
            else:
                interpret = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-{func}.npy')
            reducer = reducers[opt.interpret_method]
            clf = ensembled_clf.find_clf(opt.interpret_method)
            size = interpret.shape[0]
            X = reducer.reduce(interpret)
            Z = clf.decision_function(X)
            make_dir(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}')
            np.save(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-Z-{func}.npy',Z)
...
def generate_rectified(model,ds,labels,interpret_methods=['DL','LRP','VG','GBP','IG','org']):
    ...
    for lb in range(opt.classifier_classes):
        ...
        torch.save(rec_result,f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec_result-{lb}.npy')
    ...
    for func in opt.rec_methods:
        ...
        rec_result = []
        dataloader = DataLoader(dataset = dataset, batch_size=opt.val_batchsize,shuffle=False)
        for d,l in tqdm(dataloader,desc=f'wholeLine - rec - {func}'):
            d = d.to(opt.device)
            rec_img = eval(func)(d)
            rr = model(rec_img)
            rr = rr.max(1)[1]
        ...
        rec_result = torch.cat(rec_result).cpu()
        print('{} rec shape is {}'.format('wholeLine',rec.shape))
        torch.save(rec,f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{func}.npy')
        torch.save(rec_result,f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec_result-{func}.npy')

    for func in opt.rec_methods:
        for opt.interpret_method in interpret_methods:
            if opt.interpret_method == 'org':
                continue
            generate_interpret_only_rec(func)

def get_normers(interpret_methods = ['DL','LRP','VG','GBP','IG','org']):
    normers = {}
    for opt.interpret_method in interpret_methods:
        normalizer = Normalizer()
        normalizer.load(f'../classifier_pth/{opt.classifier_net}_{opt.dataset}_{opt.detect_method}_clf-ensemble/normer/{opt.interpret_method}')
...
def draw_rec_z(interpret_methods = ['DL','LRP','VG','GBP','IG','org'],reverse_list=[]):
    ...
    for lb in range(opt.classifier_classes):
        ...
        Rec[lb] = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{lb}.npy')
        Rec_res[lb] = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec_result-{lb}.npy').cpu()
        for opt.interpret_method in interpret_methods:
            Z_tmp = np.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-Z-{lb}.npy')
...
def get_rf(train=True):
    if train:
        clf = tree.DecisionTreeClassifier()
        return clf
    else:
        clf = pk_load(f'../classifier_pth/{opt.classifier_net}_{opt.dataset}_{opt.detect_method}_clf-ensemble/rectify_rf.pth')
        return clf

def save_rf(clf):
    pk_dump(clf,f'../classifier_pth/{opt.classifier_net}_{opt.dataset}_{opt.detect_method}_clf-ensemble/rectify_rf.pth')

def fit_rf(clf,Z_adv,label,interpret_methods):
    size = label.shape[0]
    label = label.numpy()
    inp = []
    for i in range(size):
        ...
        for lb in range(opt.classifier_classes):
            for i_method in interpret_methods:
                tmp.append(Z_adv[(lb,i_method)][i])
        for func in opt.rec_methods:
            for i_method in interpret_methods:
                tmp.append(Z_adv[(func,i_method)][i])
        inp.append(tmp)
    inp = np.array(inp)
    clf.fit(inp,label)
    return clf

def pred_rf(clf,Z_adv,size,interpret_methods):
    inp = []
    for i in range(size):
        tmp = []
        for lb in range(opt.classifier_classes):
...
def test_to_rec(reducers,ensembled_clf,clf_method,th,dataset,label,model,interpret_methods = ['DL','LRP','VG','GBP','IG','org'],reverse_list=[],top_count=5):
    ...
    for func in opt.rec_methods:
        print('loading data',func)
        Rec[func] = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{func}.npy')
        Rec_res[func] = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec_result-{func}.npy').cpu()
        for opt.interpret_method in interpret_methods:
            Z_tmp = np.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-Z-{func}.npy')
            Z_adv[(func,opt.interpret_method)] = normers[opt.interpret_method].get(Z_tmp,method = 'mean-std',reverse=reverse[opt.interpret_method])
    ...
    best = torch.zeros(size, dtype=torch.bool)
    for lb in range(opt.classifier_classes):
        best[Rec_res[lb]==label] = True
    for func in opt.rec_methods:
        best[Rec_res[func]==label] = True
    print('best condition is {:.4f}'.format(torch.sum(best).item()/size))

    not_rectified = torch.zeros(size, dtype=torch.bool)
    rec_true_tensor = torch.zeros(size, dtype=torch.bool)
    for i in range(size):
        ZZ = []
        last = None
        if clf_method == 'min':
            for lb in range(opt.classifier_classes):
                ...
                # if last < th:
                #     continue
                ZZ.append((min(Z),Rec_res[lb][i]))
            for func in opt.rec_methods:
                Z = []
                for opt.interpret_method in interpret_methods:
                    Z.append(Z_adv[(func,opt.interpret_method)][i])
                if AdvResult[i] == Rec_res[func][i]:
                    continue
                last = min(Z)
                # if last < th:
                    # continue
                ZZ.append((min(Z),Rec_res[func][i]))
            ...
            for func in opt.rec_methods:
                ...
                # if last > th:
                    # continue
                ZZ.append((max(Z),Rec_res[func][i]))
            ZZ.sort()
        ...
        for z,r in ZZ:
            ...
            if r == AdvResult[i]:
                top+=1
                continue
            not_rectified[i] = False
            if r == label[i]:
                rec_true_tensor[i] = True
            top+=1
    drop = torch.sum(not_rectified).item()
    ans = {
        'detected_total_count' : size,
        'rectified_count': size-drop,
        'true_count': torch.sum(rec_true_tensor).item(),
        'rec_true':torch.sum(rec_true_tensor).item(),
        'nr_true':0
    }
    ans['rec_true/rectified_count'] = ans['rec_true']/ans['rectified_count']  if ans['rectified_count']!=0  else  0
    ans['nr_true/(detected_total_count-rectified_count)'] = ans['nr_true']/(size-ans['rectified_count']) if (size-ans['rectified_count'])!=0  else 0
    ans['true_count/detected_total_count'] = ans['true_count']/size if size!=0 else 0

    for func in opt.rec_methods:
        tmp = (Rec_res[func]==label)
        tmp[not_rectified] = False
        ans[func] = tmp.sum().item()  
...
def main(interpret_methods = ['DL','LRP','VG','GBP','IG','org'],reverse_list=[], train = True):
    ...
    # choose_df.to_csv(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/choose.csv',float_format = '%.3f')

    item = test_to_rec(reducers,ensembled_clf,clf_method,th,to_rec_dataset,to_rec_label,model,interpret_methods,reverse_list,top_count=opt.top_count)
    item['remainted_acc'],item['remained_true'],item['remained_size'] = test_to_rem(to_rem_dataset,to_rem_label,model)
    item['rate'] = (item['rec_true']+item['remained_true'])/(item['remained_size']+item['rectified_count'])

    for func in opt.rec_methods:
        item[f'{func}_rate'] = (item[func]+item['remained_true'])/(item['remained_size']+item['rectified_count'])

    item2 = test_to_rec(reducers,ensembled_clf,clf_method,th,to_rec_dataset,to_rec_label,model,interpret_methods,reverse_list,top_count=opt.top_count*2)
    item2['remainted_acc'],item2['remained_true'],item2['remained_size'] = test_to_rem(to_rem_dataset,to_rem_label,model)
    item2['rate'] = (item2['rec_true']+item2['remained_true'])/(item2['remained_size']+item2['rectified_count'])

    for func in opt.rec_methods:
        item2[f'{func}_rate'] = (item2[func]+item2['remained_true'])/(item2['remained_size']+item2['rectified_count'])

    return item,item2

def clear_data():
    for phase in ['train','test']:
        d = f'../{opt.tmp_dataset}/{phase}/{opt.attack}_{opt.attack_param}'
        for fname in os.listdir(d):
            if 'wholeLine' in fname:
                os.remove(os.path.join(d,fname))
        for i in opt.interpret_methods:
            d2 = os.path.join(d,i)
...
if __name__ == '__main__':
    ...
    opt.rec_method = 'PGD-T'
    opt.pgd_eps_rec = 0.01
    opt.pgd_eps_iter_rec = 0.01
    opt.pgd_iterations_rec = 2
    ...
    opt.top_count = 5
    opt.rec_methods = ["gaussian","smoothing","breduce","scale","flip","rotate","fft"]
    for opt.classifier_net,opt.dataset,opt.image_channels,opt.attack_box,attack_list in [
        # ('wide_resnet','imagenet',3,'white',[("PGD-U",0.007)]),
        ('vgg11bn','cifar10',3,'white',[("CW-U",1),("CW-T",15),("PGD-U",0.007),("PGD-T",0.007),("FGSM-U",0.005),('DFool-U',0.05),("DDN-U",1000),("DDN-T",1000),]),
        ('wide_resnet','imagenet',3,'white',[("PGD-U",0.007),("PGD-T",0.007),("FGSM-U",0.05),('DFool-U',0.05),("DDN-U",1000),("DDN-T",1000),("CW-T",9),("CW-U",9)]),
    ]:
        if "imagenet" in opt.dataset:
            opt.classifier_classes=30
            opt.rec_max = 500
            ...
            opt.train_batchsize = opt.val_batchsize = 8
        if "cifar" in opt.dataset:
            opt.rec_max = 5000
            opt.train_batchsize = opt.val_batchsize = 64
            opt.detect_method="iforest"
        ...
        auc_values_train2 = {}
        train=True
        if opt.classifier_net == 'vgg11bn' and opt.dataset == 'cifar10':
            train=True
        if "black" in opt.attack_box:
            opt.max_num=500
        ...
        for opt.attack,opt.attack_param in attack_list:
            ...
            try:
                _,_,_,_,_,_,_ = get_sift_data(opt)
            except Exception as e:
                print(e)
                print(f"{opt.dataset}_{opt.classifier_net}_{opt.attack}_{opt.attack_param}_{opt.data_phase} initial generate")
                generate_all()
            item,item2 = main(train=train,interpret_methods=opt.interpret_methods,reverse_list=opt.reverse_list)
            print(item)
            print(item2)
            auc_values_train[f"{opt.attack}_{opt.attack_param}"] = item
            auc_values_train2[f"{opt.attack}_{opt.attack_param}"] = item2
            ...
            try:
                ...
                print(f"{opt.dataset}_{opt.classifier_net}_{opt.attack}_{opt.attack_param}_{opt.data_phase} initial generate")
                generate_all()
            item,item2 = main(train=False,interpret_methods=opt.interpret_methods,reverse_list=opt.reverse_list)
            print(item)
            print(item2)
            auc_values[f"{opt.attack}_{opt.attack_param}"] = item
            auc_values2[f"{opt.attack}_{opt.attack_param}"] = item2
            # draw_rec_z(interpret_methods=opt.interpret_methods,reverse_list=opt.reverse_list)
        ...
        auc_values_train.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_train_whole-top-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
        print(auc_values)
        auc_dict = pd.DataFrame.from_dict(auc_values)
        auc_dict.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_test_whole-top-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')

        print(auc_values_train2)
        auc_values_train2 = pd.DataFrame.from_dict(auc_values_train2)
        timeStamp = datetime.now()
        formatTime = timeStamp.strftime("%m-%d %H-%M-%S")
        auc_values_train2.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_train_whole-top2-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
        print(auc_values2)
        auc_dict = pd.DataFrame.from_dict(auc_values2)
        auc_dict.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_test_whole-top2-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
