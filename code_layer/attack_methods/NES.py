...
class NESAttack(Attack, LabelMixin):
    """
    ...
    def __init__(
            ...
            eps_iter=0.01, rand_init=True, clip_min=0., clip_max=1.,gradient=False,
    ...
    def perturb(self, x, y,pair,choose):
        """
        ...
        rval = self.perturb_iterative(
            x, y, self.predict,choose, nb_iter=self.nb_iter,
    ...
    def estimate_gradient(self,xvar,yvar,predict,loss_fn,sigma=1e-3):
        sample_batch_size=self.sample_batch_size
        device=xvar.device
        g=torch.zeros_like(xvar,device=device)
        size=xvar.shape
        for i in range(sample_batch_size):
            u=torch.randn(*size,device=device)
            inp=(xvar+sigma*u).detach().clone()
            outputs=predict(inp)
            loss=loss_fn(outputs,yvar).view(-1,1,1,1).repeat(1,size[1],size[2],size[3]).detach()
            g=g+loss*u
            inp=(xvar-sigma*u).detach().clone()
            outputs=predict(inp)
            loss=loss_fn(outputs,yvar).view(-1,1,1,1).repeat(1,size[1],size[2],size[3]).detach()
            g=g-loss*u
            del loss
        g=g/(2*sample_batch_size*sigma)
        return g



    def perturb_iterative(self, xvar, yvar, predict,choose, nb_iter, eps, eps_iter, loss_fn,
        ...
        all_acc=[]
        choose_sz=torch.sum(choose).item()
        ...
        for ii in tqdm(range(nb_iter)):
            ...
            if ord == np.inf:
                if self.gradient:
                    ...
                    grad = self.estimate_gradient(xvar+delta,yvar,predict,self.loss_fn)
                grad_prev=grad
            ...
            if minimize:
                acc=torch.sum(out_labels[choose]==yvar[choose]).item()
            else:
                acc=torch.sum(out_labels[choose]!=yvar[choose]).item()
...
class NES_attacker:
    def __init__(self,opt,model,gradient=False):
        ...
        self.adversary = NESAttack(model, num_classes, eps=self.eps, nb_iter=self.nb_iter,
         eps_iter=self.eps_iter, targeted=istargeted,sample_batch_size=self.sample_batch_size)
        print(f'using attacker:NES  params:eps={self.eps},lr={self.eps_iter},nb_iter={self.nb_iter},istargeted={istargeted}')

    def __call__(self,org,org_l,pair,pair_l):
        choose=self.model(org).max(1)[1]==org_l
        if self.istargeted:
            label=pair_l
        else:
            label=org_l
        image=self.adversary.perturb(org, label,pair,choose)
        self.count=self.nb_iter*2*self.sample_batch_size*org.shape[0]
        return image
