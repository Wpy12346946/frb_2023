...
def generate_interpret_only(opt):
    ...
    for data, target in tqdm(loader, desc=opt.data_phase):
        ...
        for opt.interpret_method in interpret_methods:
            ...
            interpreter.release()
            saliency_images = saliency_images * 50
            inp = saliency_images[0].cpu()
            numpy_image = inp.numpy()
            numpy_image = numpy_image.transpose((1, 2, 0))
            plt.imshow(numpy_image)
            plt.show()
            # inp = torchvision.utils.make_grid(saliency_images[0:1])
            # inp = inp.cpu()
...
if __name__ == '__main__':
    ...
    for opt.classifier_net, opt.dataset, opt.image_channels, opt.attack_box, attack_list in [
        ...
        if "cifar" in opt.dataset:
            opt.rec_max = 5000
            opt.train_batchsize = opt.val_batchsize = 2
            opt.detect_method = "iforest"
            opt.reducer = "PCA_Reducer"
        ...
        for opt.attack, opt.attack_param in attack_list:
            ...
            interpret_methods = ['DL', 'VG', 'GBP', 'IG']
            # interpret_methods = ['LRP']
            for opt.interpret_method in interpret_methods:
                generate_interpret_only(opt)
            # generate()
            try:
                ...
                print(e)
                print(
                    f"{opt.dataset}_{opt.classifier_net}_{opt.attack}_{opt.attack_param}_{opt.data_phase} initial generate")
                generate_all()
        ...
        auc_values_train.to_csv(
            f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_train_whole-{opt.rec_method}--{formatTime}.csv',
            float_format='%.3f')
        print(auc_values)
        auc_dict = pd.DataFrame.from_dict(auc_values)
        auc_dict.to_csv(
            f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_test_whole-{opt.rec_method}--{formatTime}.csv',
            float_format='%.3f')
