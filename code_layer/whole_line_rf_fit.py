...
def test_to_rec(reducers,ensembled_clf,clf_method,th,dataset,label,model,interpret_methods = ['DL','LRP','VG','GBP','IG','org'],reverse_list=[],train_rf=False):
    ...
    if(train_rf):
        clf = get_rf(train_rf)
        clf = fit_rf(clf,Z_adv,label,interpret_methods)
        save_rf(clf)
    else:
        clf = get_rf(train_rf)
    ...
    rf_pred = pred_rf(clf,Z_adv,size,interpret_methods)
    for i in range(size):
        lb = rf_pred[i]
        target_lbs[i] = Rec_res[lb][i]
        datarec.append(Rec[lb][i:i+1])
...
def main(interpret_methods = ['DL','LRP','VG','GBP','IG','org'],reverse_list=[], train = True):
    ...
    item = test_to_rec(reducers,ensembled_clf,clf_method,th,to_rec_dataset,to_rec_label,model,interpret_methods,reverse_list,train_rf = opt.data_phase=='train')
...
if __name__ == '__main__':
    ...
    opt.pgd_eps_rec = 0.005
    opt.pgd_eps_iter_rec = 0.005
    opt.pgd_iterations_rec = 20
    ...
    for opt.classifier_net,opt.dataset,opt.image_channels,opt.attack_box,attack_list in [
        ...
        auc_values_train.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_train_whole-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
        print(auc_values)
        auc_dict = pd.DataFrame.from_dict(auc_values)
        auc_dict.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_test_whole-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
