...
def generate_interpret_only_rec(func):
    if opt.interpret_method == 'org':
        return
    # Initialize the network
    # choose classifier_net in package:models
    ...
    dataset = torch.load(f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/wholeLine-Rec-{func}.npy')
    interpret = []
    
    dataset = TensorDataset(dataset)
    dataloader = DataLoader(dataset = dataset, batch_size=opt.val_batchsize,shuffle=False)

    for data, in tqdm(dataloader, desc=f'generate_interpret_only_rec-{func}'):
        # Send the data and label to the device
    ...
    torch.save(n,f'../{opt.tmp_dataset}/{opt.data_phase}/{opt.attack}_{opt.attack_param}/{opt.interpret_method}/wholeLine-Rec-{func}.npy')
    return
...
if __name__ == '__main__':
    ...
    opt.pgd_eps_rec = 0.007
    opt.pgd_eps_iter_rec = 0.007
    opt.pgd_iterations_rec = 7
    ...
    # opt.rec_methods = ["flip"]
    # print(eval(opt.rec_methods[0]))
    ...
    for opt.classifier_net,opt.dataset,opt.image_channels,opt.attack_box,attack_list in [
        ...
        ('vgg11bn','cifar10',3,'white',[("PGD-U",0.007),("PGD-T",0.007),("FGSM-U",0.005),('DFool-U',0.05),("DDN-U",1000),("DDN-T",1000),("CW-T",15),("CW-U",1),]),
    ]:
        if "imagenet" in opt.dataset:
            opt.classifier_classes=30
            opt.rec_max = 500
        ...
        auc_values_train.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_train_whole-aj-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
        print(auc_values)
        auc_dict = pd.DataFrame.from_dict(auc_values)
        auc_dict.to_csv(f'../{opt.tmp_dataset}/attack_eps/{opt.dataset}_{opt.classifier_net}_test_whole-aj-{opt.rec_method}--{formatTime}.csv',float_format = '%.3f')
